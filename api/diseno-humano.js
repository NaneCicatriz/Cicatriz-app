...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Coordenadas de la ciudad (para el cálculo de carta natal en el frontend)
    const latCiudad = loc.latitude ?? loc.lat ?? null;
    const lonCiudad = loc.longitude ?? loc.lng ?? loc.lon ?? null;
    ...
    if (hdResult.status !== 200) {
      return res.status(200).json({ error: 'Error al calcular el diseño', detalle: hdResult.body, timezone: offsetStr, lat: latCiudad, lon: lonCiudad });
    }
  ...
  firma: inf.firma,
    ...
    return res.status(200).json({ diseno: resumen, ciudad_encontrada: loc.name || ciudad, timezone: offsetStr, lat: latCiudad, lon: lonCiudad });
